...
\placebookmarks[DANYUAN,KEWEN,chapter,section,subsection] % 可加入自定义\definelist[mylist]
% 设置引用
...
\def\Insert[#1,#2]#3[#4]{%
    ...
    \offset[width=0em,y=- #2 em,location={right,top}]
        {\framedtext[width=#1 em,frame=off,offset=0em,frameoffset=0em,style={\itx\setupinterlinespace[line=1em]}]{#4 {#3}}%
    }%
}
...
% \startluacode
% function MP.lianxian(char,words) --lua.MP是给用户预留的命名空间，lua.mp则是系统自用的
%     local code1 = [[
%     draw
...
% --function userdata.lianxian(keywords, str)
