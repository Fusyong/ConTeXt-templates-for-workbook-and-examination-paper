...
\defineframedtext[bianzhu]
    ...
    frameoffset=0em, % 外框向外偏置（以上两项不影响与周围文本的距离）
...
% \def\pinyinge[#1][#2]{%
%     % \startuseMPgraphic{tianzige}
%     % \stopuseMPgraphic
%     % \useMPgraphic{tianzige}
%     \startMPcode
%         % \includeMPgraphic{tianzige} ;
%         pickup pencircle scaled 1pt ;
%         path p ; p := fullsquare scaled 1.3cm ;
%         draw p ;
%         % string s ; s := runscript("string.utfcharacters([=[#1]=])");
%         % string s ; s := lua("print('Hi')");
%         label.top(textext("#1"), p) ;
%         label(textext("\itd #2"), origin) ;
%     \stopMPcode
% }
