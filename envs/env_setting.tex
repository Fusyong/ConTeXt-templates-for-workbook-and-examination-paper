...
% samepage在某些地方不起效，而再手动加则有效 TODO
...
% \setuppapersize[standing][A3] % A3纸张打印，能看到裁切线
...
\setuplist[danyuan][width=10mm, style=bold]
\setuplist[kewen][width=10mm]
% pdf交互/链接
...
\def\datiformat#1{\par{\secondColor{#1}、}}% 在答案纸中的格式
\definehead[DATI][subsubsection]
    [%
    before={\smallblank \incrementcounter[datinumber] \incrementuniquename},
    %获取实际的值
    % 获取计数器的引用，必须在编号标题中或其后后才能显示，如 subsubsubject{#1、}
    % \saveAnswer[datiformat]{\ref[number][\uniquename]}%
    % 禁止断行要放在最后，TODO 但某些地方仍无效，需要手动再加
    after={\saveAnswer[datiformat]{\structurenumber} \smallblanksamepage},
    ]
% 实例
% 因为reference和ownnumber不能在定义/设置中应用
% reference={\uniquename},
% 转换计数器的数值为中文数码，以强制替换大题编号
\def\dati#1{%
    \DATI[title={#1}, ownnumber={\convertnumber{cn}{\rawcountervalue[datinumber]}}]%
}
