...
\definecolor[proofTextColor]  [m=1,t=0.8,a=1] % 校对文本色+透明
\definecolor[transparentColor][k=0,t=0,a=1] % 透明色（隐藏文本用）
\doifmodeelse{proofread} % 校对标记专用色（非校对模式下为透明）
    {\definecolor[proofColorOrTrans][proofTextColor]}
    {\definecolor[proofColorOrTrans][transparentColor]}
...
\def\answerS#1{%
    \setbox0=\hbox{#1}%
    \copy0\kern-\wd0\hbox to \wd0{\hfil\proofColorOrTrans\backslash\hfil}%
}

% 回答序号
\def\answerN{%
    \raise 1em \vbox to 0em{\hbox to 0em {\itx\proofColorOrTrans a.n.}}
}

% 括号
% TODO 测量宽度；允许输入宽度
\def\brackets#1{（\enskip {\proofColorOrTrans #1} \enskip）}
% 行末括号
...
% ！！！字色覆盖线色（目前用首位加全角空格的临时方法）
% 可能是标点模块删除标点前后的空白造成的
% 临时处理方法是只在填空前面加kern
\unexpanded\definebar[clozeBar][underbar][continue=yes, color=secondColor, offset=-0.5]
\def\cloze#1{%
    \startbar[clozeBar]%
    % !!! 透明度会传递到下画线
    \color[transparentColor]{#1}\color[proofColorOrTrans]{#1}%　
    % 下面的方法会导致断句困难
...
% \definebar[ClozeBar][underbar][continue=yes, color=secondColor, offset=-0.5]
