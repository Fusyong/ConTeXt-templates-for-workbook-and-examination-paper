...
% 原样输出
\def\answerAsItIs#1{#1}

% 放置缓存内容
\def\placebuffer#1{\csname get#1\endcsname}

% 自定义标题
% 单元
\def\danyuanname#1{\section{#1}}
\def\danyuan#1{%
    \chapter{#1}%
    \saveAnswer[danyuanname]{#1}%
}
% 课文标题
\def\kewenname#1{\subsubject{#1}}
\def\kewen#1{%
    \section{#1}%
    \saveAnswer[kewenname]{#1}%
}
% 大题目
% TODO 
\def\datiname#1{\par{\itc #1}、}
\def\dati#1{%
    \incrementuniquename%
    \subsubsection[\uniquename]{#1}%
    \saveAnswer[datiname]{\structurenumber}%获取实际的值
    % 获取计数器的引用，必须在编号标题中或其后后才能显示，如\subsubsubject{#1、}
    % \saveAnswer[datiname]{\ref[number][\uniquename]}%
}
