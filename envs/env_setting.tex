...
% 自订标题
\definehead[centersection][section][align=center]
\definehead[leftsection][section][align=flushleft]
\define[2]\essayhead{\centersection{#1}\middlealigned{\itx #2}\blank}
...
\def\centralTitle[#1][#2]{%
    \subsubsubject{\tf \midaligned{\color[middlecyan]{ #1 \break #2}}}%
}
...
% 完形填空  ！！！标点符号处理过程导致下划线丢失
\definebar[clozeBar][underbar][continue=yes, color=black, offset=-0.5]
\def\cloze #1{%
    \startbar[clozeBar]%
    \scratchdimen\widthofstring{#1}\relax % 获取文本宽度
    \zwj\kern.5\scratchdimen \color[black]{#1} \kern.5\scratchdimen\zwj % U+200D ZERO WIDTH JOINER
    \stopbar%
}%
