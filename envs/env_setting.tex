...
\definecolor [secondColor][c=1] % 双色中的彩色
\definecolor [aboveColor][c=1,t=0.8,a=1] % 双色中的彩色+透明（用在文字上方）
\definecolor [transparentColor][k=1,t=0,a=1] % 全透明色（隐藏文本用）
\doifmodeelse{proofread} % 校对标记专用色（非校对模式下为透明）
    {\definecolor[proofColor][aboveColor]}
    {\definecolor[proofColor][transparentColor]}
...
\def\answerS#1{%
    \setbox0=\hbox{#1}%
    \copy0\kern-\wd0\hbox to \wd0{\hfil\proofColor\backslash\hfil}%
}

% 回答序号
\def\answerN{%
    \raise 1em \vbox to 0em{\hbox to 0em {\itx\proofColor a.n.}}
}

% 括号
% TODO 测量宽度；允许输入宽度
\def\brackets#1{（\enskip {\proofColor #1} \enskip）}
% 行末括号
